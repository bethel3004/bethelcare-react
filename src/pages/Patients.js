              ...
              {selected?.ID === p.ID && (
                <div className="card-body" style={{ borderTop:'1px solid var(--border2)' }}>
                    ...
                    <div>
                      ...
                      {(p.혈압_입소시 || p.혈당_입소시) && (
                        <div style={{ display:'flex', gap:8, marginBottom:10 }}>
                          {p.혈압_입소시 && <div style={{ flex:1, background:'var(--red-bg)', borderRadius:8, padding:'8px 12px', textAlign:'center' }}>
                            <div style={{ fontSize:'0.65rem', color:'var(--red)', fontWeight:700 }}>입소시 혈압</div>
                            <div style={{ fontSize:'0.9rem', fontWeight:700, color:'var(--red)', marginTop:2 }}>{p.혈압_입소시}</div>
                          </div>}
                          {p.혈당_입소시 && <div style={{ flex:1, background:'var(--amber-bg)', borderRadius:8, padding:'8px 12px', textAlign:'center' }}>
                            <div style={{ fontSize:'0.65rem', color:'var(--amber)', fontWeight:700 }}>입소시 혈당</div>
                            <div style={{ fontSize:'0.9rem', fontWeight:700, color:'var(--amber)', marginTop:2 }}>{p.혈당_입소시}</div>
                          </div>}
                        </div>
                      )}
                  ...
                  <div style={{ display:'flex', justifyContent:'flex-end', marginTop:14 }}>
                    <button className="btn btn-danger btn-sm" onClick={()=>{ setPatients(patients.filter(x=>x.ID!==p.ID)); setSelected(null); }}>삭제</button>
                  </div>
                </div>
              )}
            </div>
