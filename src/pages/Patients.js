...
function validAdmissions(str) {
  ...
  return str.split('\n').map(s => s.trim()).filter(s => {
    ...
    if (!m) return false;
    const [, y1,,d1, y2,,d2] = m;
    if (parseInt(y1) < 2020 || parseInt(y2) < 2020) return false;
    if (d1 === '00' || d2 === '00') return false;
    return true;
  });
}
...
export default function Patients({ db }) {
  ...
  const initState = () => {
    ...
    return {
      search: p.search||'', statusFilter: p.status||'전체',
      campFilter: p.camp||'전체', tab: p.tab||'list', selectedId: p.patient||null
    };
  };
  const [urlState, setUrlState] = useState(initState);
  const { search, statusFilter, campFilter, tab, selectedId } = urlState;

  // ── ✅ 핵심 수정: selectedId 기반으로 selected 계산 ──────────
  const selected = patients.find(p => p.ID === selectedId) || null;

  const updateState = useCallback((next) => {
    setUrlState(prev => {
      const merged = { ...prev, ...next };
      pushHashParams('patients', {
        search: merged.search, status: merged.statusFilter,
        camp: merged.campFilter, patient: merged.selectedId||'', tab: merged.tab
      });
      return merged;
    });
  }, []);
  ...
  const setSelected = useCallback((p) => {
    updateState({ selectedId: p ? p.ID : null });
  }, [updateState]);
  ...
  const handleEdit = (e) => {
    ...
    setEditTarget(null); setEditForm(null);
    setAdmissions([{ start:'', end:'' }]);
    updateState({ tab:'list', selectedId: null });
  };
            ...
            <div style={{display:'flex',gap:4,flexWrap:'wrap',alignItems:'center'}}>
              {['전체','입소중','퇴소'].map(s=>(
                <button key={s} onClick={()=>updateState({statusFilter:s})}
                  className={statusFilter===s?'btn btn-primary btn-sm':'btn btn-ghost btn-sm'}>{s}</button>
              ))}
              <select value={campFilter} onChange={e=>updateState({campFilter:e.target.value})}
                className="form-select" style={{height:32,padding:'0 8px',fontSize:'0.85rem',minWidth:100}}>
                  ...
                  <div style={{display:'flex',flexDirection:window.innerWidth<=767?'column':'row',gap:20,alignItems:'flex-start'}}>
                    <div style={{flex:'0 0 auto',width:window.innerWidth<=767?'100%':'38%'}}>
                      ...
                      {[
                        ['캠프장소', p.캠프장소],
                        validAdmissions(p.입소기간).slice(-1)[0]
                          ? ['최근 입소기간', validAdmissions(p.입소기간).slice(-1)[0]] : null,
                    ...
                    <div style={{flex:1,width:window.innerWidth<=767?'100%':'auto'}}>
                      ...
                      <div style={{fontSize:'0.8rem',color:'var(--text2)',lineHeight:1.6,background:'var(--bg)',borderRadius:8,padding:'10px 12px',whiteSpace:'pre-wrap',wordBreak:'break-word'}}>
                        {(getTreatment(p)||'없음').replace(/ > /g,'\n> ')}
                      </div>
                    </div>
                  </div>
                  {!isGuest && (
                    <div style={{display:'flex',justifyContent:'flex-end',gap:8,marginTop:14}}>
                      <button className="btn btn-ghost btn-sm" onClick={()=>{
                        setEditTarget(p);
                        setEditForm({...p});
                        setAdmissions(parseAdmissions(p.입소기간));
                        setSelected(null);
                        updateState({tab:'edit', selectedId:null});
                      }}>✏️ 수정</button>
                      <button className="btn btn-danger btn-sm" onClick={()=>{
                        if (p._rowIndex) deleteFromSheet('입소자', p._rowIndex);
                        setPatients(patients.filter(x=>
                          x._rowIndex ? x._rowIndex !== p._rowIndex : (x.ID ? x.ID !== p.ID : x !== p)
                        ));
                        setSelected(null);
                      }}>🗑️ 삭제</button>
                    </div>
          ...
          <form className="card-body" onSubmit={handleEdit}>
            ...
            <div className="form-grid form-grid-3" style={{marginBottom:16}}>
              ...
              <div className="form-group"><label className="form-label">성별</label>
                <select className="form-select" {...ef('성별')}><option>여</option><option>남</option></select></div>
            ...
            <div className="form-grid form-grid-2" style={{marginBottom:16}}>
              ...
              <div className="form-group" style={{gridColumn:'1/-1'}}><label className="form-label">비고</label><input className="form-input" {...ef('비고')}/></div>
            </div>
            <div className="section-label">의료사항</div>
            <div className="form-grid form-grid-2" style={{marginBottom:16}}>
              ...
              <div className="form-group" style={{gridColumn:'1/-1'}}><label className="form-label">치료 경력</label>
                <textarea className="form-textarea" rows={14} style={{minHeight:320,whiteSpace:'pre-wrap',wordBreak:'break-word'}} {...ef('치료경력')}/></div>
              <div className="form-group" style={{gridColumn:'1/-1'}}><label className="form-label">입소기간</label>
                <AdmissionPicker initialRows={parseAdmissions(editTarget?.입소기간||'')} onChange={setAdmissions}/></div>
              <div className="form-group"><label className="form-label">상태</label>
                <select className="form-select" {...ef('상태')}><option>입소중</option><option>퇴소</option></select></div>
            </div>
        ...
        <div className="card">
          ...
          <form className="card-body" onSubmit={handleAdd}>
            ...
            <div className="form-grid form-grid-3" style={{marginBottom:16}}>
              ...
              <div className="form-group"><label className="form-label">성별</label>
                <select className="form-select" {...f('성별')}><option>여</option><option>남</option></select></div>
            ...
            <div className="form-grid form-grid-2" style={{marginBottom:16}}>
              ...
              <div className="form-group" style={{gridColumn:'1/-1'}}><label className="form-label">치료 경력</label>
                <textarea className="form-textarea" rows={14} style={{minHeight:320,whiteSpace:'pre-wrap',wordBreak:'break-word'}} placeholder="수술력, 복약이력 등" {...f('치료경력')}/></div>
              <div className="form-group" style={{gridColumn:'1/-1'}}>
                <label className="form-label">입소기간</label>
                <AdmissionPicker initialRows={[]} onChange={v=>setForm({...form,입소기간:v})}/>
              </div>
              <div className="form-group"><label className="form-label">상태</label>
                <select className="form-select" {...f('상태')}><option>입소중</option><option>퇴소</option></select></div>
            </div>
            <div className="form-actions">
